...
def main_menu():  # функция главного меню - выбора из списка функций
    ...
    tk.Label(frame1, text="Enter a function:", font=("Calibri", 14)).grid(column=0, row=0, pady=0,
                                                                          padx=0)  # выводит на фрейм надпись
    ...
    tk.Button(frame1, text="Convolution reconstruction", font=("Calibri", 14),
              command=holography_reconstruction_convolution).grid(column=0, row=4, pady=10, padx=25)

    tk.Button(frame1, text="Calculation", font=("Calibri", 14), command=synthetic_wavelength_function).grid(column=0,
                                                                                                            row=6,
                                                                                                            pady=10,
                                                                                                            padx=25)

    tk.Button(frame1, text="Convert CSV to JPG", font=("Calibri", 14), command=convert_csv_to_jpg).grid(column=0, row=8,
                                                                                                        pady=10,
                                                                                                        padx=25)

    tk.Button(frame1, text="Average picture", font=("Calibri", 14), command=average_picture).grid(column=0, row=10,
                                                                                                  pady=10, padx=25)

    tk.Button(frame1, text="Phase difference", font=("Calibri", 14), command=phase_difference_function).grid(column=0,
                                                                                                             row=12,
                                                                                                             pady=10,
                                                                                                             padx=25)

    tk.Button(frame1, text="3D-reconstruction", font=("Calibri", 14), command=topography).grid(column=0, row=14,
                                                                                               pady=10, padx=25)

    tk.Button(frame1, text="Determination of recovery distance", font=("Calibri", 14), command=recovery_distance).grid(
        column=0, row=16, pady=10, padx=25)

    tk.Button(frame1, text="Hologram generation", font=("Calibri", 14), command=hologram_generation).grid(column=0,
                                                                                                          row=18,
                                                                                                          pady=10,
                                                                                                          padx=25)

    tk.Button(frame1, text="Exit", font=("Calibri", 14), command=window.destroy).grid(column=0, row=20, pady=10,
                                                                                      padx=25)
...
def save_files():  # Проверка файлов во всей папке, если выявляется файл с таким же названием, то запрашивается разрешение перезаписи
    ...
    if os.path.exists(name_picture_text_input.get() + '_intensity.png') or os.path.exists(
            name_picture_text_input.get() + '_phase.png') or os.path.exists(
            name_picture_text_input.get() + '_intensity.csv') or os.path.exists(
            name_picture_text_input.get() + '_phase.csv'):  # проверка, что файла с такими названиями нет в папке
...
def save_image_and_csv_files():  # подфункция, которая сохраняет изображения и матрицы
    ...
    plt.imsave(name_picture_text_input.get() + '_intensity.png', np.power(intensity, 0.4),
               cmap='gray')  # сохранение изображения
    ...
    messagebox.showinfo('Reconstruction',
                        'Reconstruction complete!')  # сообщение на экране, что реконструкция прошла успешно
...
def reconstruction_data_input():  # подфункция для ввода параметров цифрового голографического восстановления
    ...
    tk.Entry(frame2, width=20, textvariable=wav_text_input).grid(column=1,
                                                                 row=6)  # entry - добавление строки ввода (и ниже тоже)
    ...
    tk.Button(frame2, text="Back to main menu", font=("Calibri", 14), command=back_to_main_menu).grid(column=0, row=36,
                                                                                                      pady=10,
                                                                                                      padx=25)  # кнопка выхода в главное меню
...
def input_holography_reconstruction_fresnel():  # подфукнция для открытия голограммы, её обработки и ввода параметров для восстановления её методом Френеля
    ...
    tk.Label(frame4, text='                                   ').grid(column=0,
                                                                      row=0)  # создаёт пустой фрейм, чтобы название файла на экране менялось корректно
    ...
    tk.Button(frame2, text="Enter", font=("Calibri", 14), command=click_holography_reconstruction_fresnel).grid(
        column=1, row=14, pady=10, padx=25)  # запуск функции голографического восстановления методом Френеля
...
def click_hologram_generation():
    if is_number(wav_text_input.get()) == False or is_number(z0_text_input.get()) == False or is_number(
        ...
        if wav == 0 or z0 == 0 or size == 0:  # проверка на то, что все элементы - ненулевые
            ...
            tk.Button(frame2, text="OK", font=("Calibri", 14), command=save_files_hologram).grid(column=1, row=22,
                                                                                                 pady=10,
                                                                                                 padx=25)  # вызов функции сохранения восстановленных голограмм
