...
def main_menu():    #функция главного меню - выбора из списка функций
    ...
    frame1 = tk.Frame(window,width = 1000, height = 1000)    #первый фрейм с отображением главного меню
    frame2 = tk.Frame(window)    #второй фрейм с отображением выбранной функции
    frame3 = tk.Frame(window)    #В этом фрейме появляется картинка, график и т.д.
    frame4 = tk.Frame(window)    #В этом фрейме появляется надпись, какой файл открыт
    ...
    tk.Label(frame1, text = "Enter a function:", font = ("Calibri", 14)).grid(column = 0, row = 0, pady = 0, padx = 0)   #выводит на фрейм надпись

    tk.Button(frame1, text = "Fresnel reconstruction", font = ("Calibri", 14), command = holography_reconstruction_fresnel).grid(column = 0, row = 2, pady = 10, padx = 25)    #тут и дальше: button - кнопка, нажатие которой выполняет соответствующую функцию, указанную в аргументах. Метод grid размещает кнопку в "сетке"

    tk.Button(frame1, text = "Convolution reconstruction", font = ("Calibri", 14), command = holography_reconstruction_convolution).grid(column = 0, row = 4, pady = 10, padx = 25)

    tk.Button(frame1, text = "Calculation", font = ("Calibri", 14), command = synthetic_wavelength_function).grid(column = 0, row = 6, pady = 10, padx = 25)

    tk.Button(frame1, text = "Convert CSV to JPG", font = ("Calibri", 14), command = convert_csv_to_jpg).grid(column = 0, row = 8, pady = 10, padx = 25)
    
    tk.Button(frame1, text = "Average picture", font = ("Calibri", 14), command = average_picture).grid(column = 0, row = 10, pady = 10, padx = 25)
    
    tk.Button(frame1, text = "Phase difference", font = ("Calibri", 14), command = phase_difference_function).grid(column = 0, row = 12, pady = 10, padx = 25)

    tk.Button(frame1, text = "3D-reconstruction", font = ("Calibri", 14), command = topography).grid(column = 0, row = 14, pady = 10, padx = 25)
    
    tk.Button(frame1, text = "Determination of recovery distance", font = ("Calibri", 14), command = recovery_distance).grid(column = 0, row = 16, pady = 10, padx = 25)

    tk.Button(frame1, text = "Hologram generation", font = ("Calibri", 14), command = hologram_generation).grid(column = 0, row = 18, pady = 10, padx = 25)

    tk.Button(frame1, text = "Exit", font = ("Calibri", 14), command = window.destroy).grid(column = 0, row = 20, pady = 10, padx = 25)
...
def back_to_main_menu():    #подфункция возврата в главное меню
    ...
    frame2.destroy()    #фрейм 2 уничтожается, далее выполняется функция main, на котором фреймы отрисовываются заново
...
def save_files():   #Проверка файлов во всей папке, если выявляется файл с таким же названием, то запрашивается разрешение перезаписи

    if os.path.exists(name_picture_text_input.get() + '_intensity.png') or os.path.exists(name_picture_text_input.get() + '_phase.png') or os.path.exists(name_picture_text_input.get() + '_intensity.csv') or os.path.exists(name_picture_text_input.get() + '_phase.csv'):    #проверка, что файла с такими названиями нет в папке

        question = messagebox.askquestion("Warning!", "This name is already in use\nSave file?")   #выводит окно с переименованием
...
def save_image_and_csv_files():    #подфункция, которая сохраняет изображения и матрицы

    plt.imsave(name_picture_text_input.get() + '_intensity.png', np.power(intensity,0.4), cmap='gray')   #сохранение изображения
    
    plt.imsave(name_picture_text_input.get() + '_phase.png', phase, cmap='gray')
    
    CSV_file = open(name_picture_text_input.get() + '_intensity.csv', 'w')    #Сохранение матрицы интенсивности
    with CSV_file:
        writer = csv.writer(CSV_file, delimiter=';', lineterminator='\n')
        writer.writerows(intensity)    #построчная запись матрицы
    
    CSV_file = open(name_picture_text_input.get() + '_phase.csv', 'w')    #Сохранение матрицы фазы
    with CSV_file:
        writer = csv.writer(CSV_file, delimiter=';', lineterminator='\n')
        writer.writerows(phase)            

    messagebox.showinfo('Reconstruction', 'Reconstruction complete!')   #сообщение на экране, что реконструкция прошла успешно
    
    frame3.place_forget()
    

def reconstruction_data_input():    #подфункция для ввода параметров цифрового голографического восстановления
            
    global pix_text_input
    global wav_text_input
    global z0_text_input
    
    pix_text_input = tk.StringVar()    #ввод текста в строку ввода
    ...
    tk.Entry(frame2, width = 20, textvariable = wav_text_input).grid(column = 1, row = 6)    #entry - добавление строки ввода (и ниже тоже)
    ...
    tk.Button(frame2, text = "Back to main menu", font = ("Calibri", 14), command = back_to_main_menu).grid(column = 0, row = 36, pady = 10, padx = 25)    #кнопка выхода в главное меню
    

def input_holography_reconstruction_fresnel():    #подфукнция для открытия голограммы, её обработки и ввода параметров для восстановления её методом Френеля 
    ...
    tk.Label(frame4, text = '                                   ').grid(column = 0, row = 0)    #создаёт пустой фрейм, чтобы название файла на экране менялось корректно
    ...
    tk.Button(frame2, text = "Enter", font = ("Calibri", 14), command = click_holography_reconstruction_fresnel).grid(column = 1, row = 14, pady = 10, padx = 25)    #запуск функции голографического восстановления методом Френеля



def function_holography_reconstruction_fresnel():    #подфункция преобразования Френеля
...
def click_holography_reconstruction_fresnel():    #подфункция голографического восстановления методом Френеля
    
    global pix
    global wav
    global z0

    if is_number(pix_text_input.get()) == False or is_number(wav_text_input.get()) == False or is_number(z0_text_input.get()) == False:
        ...
        if pix == 0 or wav == 0 or z0 == 0:    #проверка на то, что все элементы - ненулевые
            ...
            tk.Button(frame2, text = "OK", font = ("Calibri", 14), command = save_files).grid(column = 1, row = 22, pady = 10, padx = 25)      #вызов функции сохранения восстановленных голограмм
...
def holography_reconstruction_fresnel(): #общая функция реконструкции голограмм методом Френеля
    ...
    tk.Button(frame2, text = "Open hologram", font = ("Calibri", 14), command = input_holography_reconstruction_fresnel).grid(column = 0, row = 2, pady = 10, padx = 25) 
    
    tk.Button(frame2, text = "Back to main menu", font = ("Calibri", 14), command = back_to_main_menu).grid(column = 0, row = 36, pady = 10, padx = 25)   
...
def input_holography_reconstruction_convolution():    #подфукнция для открытия голограммы, её обработки и ввода параметров для восстановления её методом свёртки
    ...
    tk.Button(frame2, text = "Enter", font = ("Calibri", 14), command = click_holography_reconstruction_convolution).grid(column = 1, row = 14, pady = 10, padx = 25)    #запуск функции голографического восстановления методом свёртки
...
def function_holography_reconstruction_convolution():    #подфункция преобразования свёрткой
    ...
    weight = 1j*np.exp(-1j*k*np.sqrt(np.power(z0,2) + np.power(gradx_1,2) + np.power(grady_1,2))) / (wav*np.sqrt(np.power(z0,2) + np.power(gradx_1,2) + np.power(grady_1,2)))
...
def holography_reconstruction_convolution(): #общая функция реконструкции голограмм методом Френеля
    ...
    tk.Button(frame2, text = "Open hologram", font = ("Calibri", 14), command = input_holography_reconstruction_convolution).grid(column = 0, row = 2, pady = 10, padx = 25) 
...
def click_synthetic_wavelength_function():    #подфункция подсчёта длины волны биений, максимальную длину восстановления, ширину восстановленного изображения в миллиметрах
    ...
    if is_number(wav_1_text_input.get()) == False or is_number(wav_2_text_input.get()) == False or is_number(pix_text_input.get()) == False or is_number(z0_text_input.get()) == False:
        messagebox.showerror('Error!', 'You entered a non-numeric value!') 
        ...
        if pix == 0 or wav_1 == 0 or wav_2 == 0 or z0 == 0:    #проверка на нулевые значения
            ...
            if  wav_1 == wav_2:    #Если две длины волны равны, то биений не будет
                ...
                L0_text = str(round(L0,3))    #считывает обработанные переменные и преобразует их в текстовые данные с округлением
...
def multiwavelength_data_input():    #ввод данных для функции вычисления длины волны биений (нужна ещё и для функции построения профиля поверхности)
    ...
    tk.Label(frame2, text = 'Wavelength 1 (mm): ').grid(column = 0, row = 4)    #новый параметр, перекрывающий изначальный из функции reconstruction_data_input()
...
def synthetic_wavelength_function():    #Функция вычисления длины волны биений двух длин волн
    ...
    tk.Button(frame2, text = "Enter", font = ("Calibri", 14), command = click_synthetic_wavelength_function).grid(column = 1, row = 14, pady = 10, padx = 25) 
...
def csv_average_picture():   #подфункция усреднения нескольких голограмм в формате *.csv
    ...
    if all_picture == []:   #если файлов нет, вывести ошибку
        ...
        for i in range(len(all_picture)):
            ...
            N_local = len(genfromtxt(all_picture[i], delimiter=';'))    #определение числа пикселей отдельного изображения в массиве изображений
        ...
        messagebox.showinfo('Averaging', 'Averaging complete!')    #вывод информационного сообщения о завершении усреднения
...
def image_average_picture():    #подфункция усреднения нескольких голограмм в формате изображений
    ...
    all_picture = [f for f in os.listdir(dir) if f.endswith('.png') or f.endswith('.jpg') or f.endswith('.bmp') ]    #массив с матрицами в формате png, jpg или bmp
    ...
    if all_picture == []:  
        ...
        for i in range(len(all_picture)):
            ...
            N_local = len(img.imread(all_picture[i]))    #определение числа пикселей отдельного изображения в массиве изображений
        ...
        for i in range(len(all_picture)):
            ...
            N = len(picture)
            np.array(picture[:,:N],dtype = float)   #обрезка изображений до квадрата и преобразование значений интенсивности в тип float
            if picture.ndim == 3:
                picture_1 =  picture[:,:,0]    #выделение одного цветового канала, если изображение имеет три цветовых канала
            else:
...
def average_picture():  #Функция усредения всех картинок или матриц, находящихся в папке (выбор типа изображений по кнопке)
    ...
    tk.Button(frame2, text = 'CSV-files', font = ("Calibri", 14), command = csv_average_picture).grid(column = 0, row = 2, pady = 10, padx = 25)
    
    tk.Button(frame2, text = 'Image files', font = ("Calibri", 14), command = image_average_picture).grid(column = 0, row = 4, pady = 10, padx = 25)
...
def save_phase_diff():   #Проверка файлов во всей папке, если выявляется файл с таким же названием, то запрашивается разрешение перезаписи
    ...
    if os.path.exists(name_phase_diff_text_input.get() + '.csv') or os.path.exists(name_phase_diff_text_input.get() + '_filtered.csv') or os.path.exists(name_phase_diff_text_input.get() + '__unwrapped_filtered.csv'):    #проверка, что файла с такими названиями нет в папке
...
def save_image_and_csv_phase_diff():    #присвоение имени файла разности фаз и сохранеие в формате изображения и матрицы *.csv
    ...
    messagebox.showinfo('Save phase difference', 'Phase difference complete!')   #вывод информационного сообщения о завершении фильтрации
...
def click_phase_difference_function():   #подфункция подсчёта разности фаз
    ...
    tk.Button(frame2, text = "OK", font = ("Calibri", 14), command = save_phase_diff).grid(column = 1, row = 22, pady = 10, padx = 25)    #кнопка, вызывающая функцию сохранения разности фаз в файл   
...
def phase_difference_function(): #Функция подсчёта разности фаз двух состояний
    ...
    tk.Button(frame2, text = 'Phase 1', font = ("Calibri", 14), command = phase_1_open).grid(column = 0, row = 2, pady = 10, padx = 25)
    
    tk.Button(frame2, text = 'Phase 2', font = ("Calibri", 14), command = phase_2_open).grid(column = 0, row = 4, pady = 10, padx = 25)
  
    tk.Button(frame2, text = "Enter", font = ("Calibri", 14), command = click_phase_difference_function).grid(column = 0, row = 8, pady = 10, padx = 25)    #кнопка запуска подфункции подсчета разности фаз  
...
def calculate_topography_function():   #подфункция вычисления трёхмерного профиля поверхности
    ...
    if is_number(wav_1_text_input.get()) == False or is_number(wav_2_text_input.get()) == False or is_number(pix_text_input.get()) == False or is_number(z0_text_input.get()) == False:
        ...
        N = len(depth_map)
        x,y = np.meshgrid(np.arange(N),np.arange(N))   #задание линейной функции возрастания для двух других осей объекта
...
def frame_topography():
    ...
    topography_image = axes.plot_surface(x*L0/N, y*L0/N, depth_map, cmap=cm.coolwarm, linewidth=0, antialiased=False)    #трёхмерная визуализация поверхности
...
def input_topography_function():
    ...
    tk.Button(frame2, text = "Window interface", font = ("Calibri", 14), command = new_window_topography).grid(column = 0, row = 12, pady = 10, padx = 25)   #кнопка для выполнения фукнции построения трёхмерного профиля поверхности в отдельном окне
    
    tk.Button(frame2, text = "TKinter interface", font = ("Calibri", 14), command = frame_topography).grid(column = 1, row = 12, pady = 10, padx = 25)   #кнопка для выполнения фукнции построения трёхмерного профиля поверхности в текущем фрейме
...
def topography(): #Функция построения топограммы поверхности
    ...
    tk.Button(frame2, text = "Open height map", font = ("Calibri", 14), command = input_topography_function).grid(column = 0, row = 2, pady = 10, padx = 25) 
...
#     root.destroy()
    ...
    if os.path.splitext(name_picture_open)[1] == '.csv':                 # делим функцией os.path.splitext() название файла на имя и расширение, и сравниваем расширение с указанным
        ...
        picture = picture_open[:,:N]                                # обрезаем ширину картинки до высоты (если высота больше ширины, то произойдет ошибка)
        
    elif os.path.splitext(name_picture_open)[1] == '.jpg' or os.path.splitext(name_picture_open)[1] == '.JPG' or os.path.splitext(name_picture_open)[1] == '.png' or os.path.splitext(name_picture_open)[1] == '.bmp':
...
def input_recovery_distance():
    ...
    tk.Button(frame2, text = "Enter", font = ("Calibri", 14), command = click_recovery_distance).grid(column = 1, row = 14, pady = 10, padx = 25)    #запуск функции голографического восстановления методом Френеля
...
def click_recovery_distance():
    ...
    if is_number(pix_text_input.get()) == False or is_number(wav_text_input.get()) == False or is_number(min_z0_text_input.get()) == False or is_number(max_z0_text_input.get()) == False or is_number(step_text_input.get()) == False:
        messagebox.showerror('Error!', 'You entered a non-numeric value!') 
        ...
        if pix == 0 or wav == 0 or max_z0 == 0 or min_z0 == 0 or step == 0:    #проверка на то, что все элементы - ненулевые
            ...
            array_intensity = np.zeros((round((max_z0 - min_z0)/step)+1))    #нулевой массив с числом элементов, равным количеству шагов по дистанции
            
            question = messagebox.askquestion("Attention", "Save reconstruction pictures?")   #выводит окно с переименованием
            ...
            for i in range(len(array_intensity)):   #цикл восстановления с разными шагами
                ...
                sum_intensity = np.sum(np.sum(intensity))   #подсчёт суммы интенсивностей всех пикселей восстановленного изображения
            ...
            array_intensity[index_min_intensity] = max(array_intensity)    #теперь минимальному элементу массива присваивается значение, например, максимального элемента, чтобы найти второй с конца минимальный элемент
...
def recovery_distance():    #фунцкия восстановления голограмм с определённым шагом по дистанции, а также позволяющая по наиболее затемнённому изображению определять дистанцию восстановления
    ...
    tk.Button(frame2, text = "Open hologram", font = ("Calibri", 14), command = input_recovery_distance).grid(column = 0, row = 2, pady = 10, padx = 25) 
...
def hologram_generation():
    ...
    tk.Button(frame2, text = "Open picture", font = ("Calibri", 14), command = input_hologram_generation).grid(column = 0, row = 2, pady = 10, padx = 25) 
...
def input_hologram_generation():
    ...
    tk.Button(frame2, text = "Enter", font = ("Calibri", 14), command = click_hologram_generation).grid(column = 1, row = 14, pady = 10, padx = 25)    #запуск функции голографического восстановления методом Френеля


def save_files_hologram():

    if os.path.exists('hologram_' + name_hologram_text_input.get() + '.png')or os.path.exists('hologram_' + name_hologram_text_input.get() + '.csv'):    #проверка, что файла с такими названиями нет в папке
...
def save_image_and_csv_hologram():
    ...
    plt.imsave('hologram_' + name_hologram_text_input.get() + '.png', hologram[size_picture:,size_picture:], cmap='gray')   
    ...
    messagebox.showinfo('Hologram generation', 'Hologram generation complete!')   #сообщение на экране, что реконструкция прошла успешно
    

def click_hologram_generation():

    if is_number(wav_text_input.get()) == False or is_number(z0_text_input.get()) == False or is_number(size_text_input.get()) == False:
        messagebox.showerror('Error!', 'You entered a non-numeric value!') 
        ...
        if wav == 0 or z0 == 0 or size == 0:    #проверка на то, что все элементы - ненулевые
            ...
            tk.Button(frame2, text = "OK", font = ("Calibri", 14), command = save_files_hologram).grid(column = 1, row = 22, pady = 10, padx = 25)      #вызов функции сохранения восстановленных голограмм
